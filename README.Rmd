...
This CV is generated using the R package [pagedown](https://rstudio.github.io/pagedown/), which uses the JavaScript library [paged.js](https://www.pagedjs.org/). This allows creating a CV based on R Markdown using markdown and HTML making it very flexible.

In addition, this CV is based on pretty much all content being stored in a Google Sheet, thus separating content and styling.
...
I wanted to update my CV, but editing a document/slide is quite a bit of manual work. That is especially as this can easily lead to re-formatting the whole file. I remembered that some people were doing CVs using Rmarkdown. On researching, I came across the [`datadrivencv`](http://nickstrayer.me/datadrivencv/) package. That seemed like a great approach as it allows me to:

* Store the content in a file making it easy to edit.
* Change the style using CSS.
* Easily create HTML and PDF versions.
* The `datadrivencv` package creates the needed files based on templates. After that my CV does not depend on the package.
* Use R (I like using R).

# Using it

Feel free to re use what I created. I made a bunch of customisations compared to what `datadrivencv` gives you. I'm planning to write a blog post about that on my blog https://gapthemind.info

You might fork my repo, copy my spreadhseet, fill it with your own data and with that render your own CV.

To help with reproducibility, this projects uses the R package `renv` to define the R packages and its versions. Use `renv::restore()` to restore the state of the project from `renv.lock`. See the [package documentation](https://rstudio.github.io/renv/articles/renv.html) for more details.

## Format of spreadsheets

The data is stored in a [Google Sheet](https://docs.google.com/spreadsheets/d/1KcTSyanKHQ1jGnwVd6AxMcb3zXLvt-2RKzsPOfmI5Cw/edit#gid=917338460)

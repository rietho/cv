...
To help with reproducibility, this projects uses the R package `renv` to
define the R packages and its versions. Use `renv::restore()` to restore
the state of the project from `renv.lock`. See the [package
documentation](https://rstudio.github.io/renv/articles/renv.html) for
more details.
...
of separate sub-sheets within a google sheet. The spreadsheets that are
needed and their columns are (note: these are partly outdated, as I did
some updates to move more deta from the R markdown file into the sheet):
